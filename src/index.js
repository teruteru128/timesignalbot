...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (message.author.bot) {
    ...
    if (YATTAZE_PATTERN.test(message.content)) {
      ...
      await message.channel.send(`投稿者：${message.member.displayName.replaceAll('@everyone', '\\@everyone')} （${now.getMonth() + 1}月${now.getDate()}日（${YOUBI[now.getDay()]}）${now.getHours().toString().padStart(2, '0')}時${now.getMinutes().toString().padStart(2, '0')}分${now.getSeconds().toString().padStart(2, '0')}秒）`);
    }
  }
});
